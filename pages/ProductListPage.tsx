    ...
    <Layout settings={settings}>
      <div className="bg-primary py-8 text-center text-white">
        <div className="max-w-7xl mx-auto px-6 md:px-4 flex items-center justify-center gap-4 relative">
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight text-center">{category.name}</h1>
        </div>
      </div>
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="py-5 px-5">
                  <h3 className="font-extrabold text-black text-sm md:text-base leading-tight line-clamp-2 text-left h-10 md:h-12">{product.name}</h3>
                </div>
              </Link>
            ))}
          </div>
